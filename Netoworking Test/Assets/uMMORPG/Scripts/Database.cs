        ...
        if (table.Count == 1) {
            ...
            if (prefab != null) {
                ...
                if (guild != null) {
                    ...
                    // load members list
                    table = ExecuteReader("SELECT character, rank FROM guild_members WHERE guild=@guild", new SqliteParameter("@guild", player.guild));
                    foreach (var row in table) {
    ...
    public static void SaveGuild(string guild, string notice, List<GuildMember> members) {
        ...
        foreach (var member in members) {
            ExecuteNonQuery("INSERT INTO guild_members VALUES(@guild, @character, @rank)",
                            ...
                            new SqliteParameter("rank", member.rankIndex));
        }
